...
function now () {
  const key = 'nowForBAPDeprecationTesting'
...
export function saveBAPPopupShown () {
  saveInteractionState({ popupShown: now() })
}

export function saveBAPAlertDismissed () {
  saveInteractionState({ alertDismissed: now() })
}
...
export function shouldShowBAPAlert (onlyAnonWallet: boolean, balance: number) {
  ...
  if (alertDismissed) {
    ...
    if (daysInPast(alertDismissed) < 3 || now() >= bapCutoffBegins) {
      return false
    }
  }
