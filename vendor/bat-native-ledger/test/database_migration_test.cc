...
using ledger::testing::GetTestDataPath;
using ledger::testing::LedgerTest;
...
TEST_F(LedgerDatabaseMigrationTest, Migration_8_PendingContribution) {
  ...
  auto pending_contribution = type::PendingContribution::New();
  ...
  pending_contribution->type = type::RewardsType::ONE_TIME_TIP;
...
TEST_F(LedgerDatabaseMigrationTest, Migration_11_ContributionInfo) {
  ...
  EXPECT_EQ(ac_sql.ColumnInt(2),
            static_cast<int>(type::RewardsType::AUTO_CONTRIBUTE));
...
TEST_F(LedgerDatabaseMigrationTest, Migration_18_CredsBatch) {
  ...
  type::CredsBatch creds_database;
  while (sql.Step()) {
    creds_database.trigger_id = sql.ColumnString(1);
    creds_database.trigger_type =
        static_cast<type::CredsBatchType>(sql.ColumnInt(2));
    ...
    creds_database.status =
        static_cast<type::CredsBatchStatus>(sql.ColumnInt(8));
  }

  type::CredsBatch creds_expected;
  creds_expected.trigger_id = "36baa4c3-f92d-4121-b6d9-db44cb273a02";
  creds_expected.trigger_type = type::CredsBatchType::PROMOTION;
...
TEST_F(LedgerDatabaseMigrationTest, Migration_21_ContributionInfoPublishers) {
  ...
  while (info_sql.Step()) {
    auto contribution_publisher = type::ContributionPublisher::New();
...
TEST_F(LedgerDatabaseMigrationTest, Migration_23_ContributionQueue) {
  ...
  type::ContributionQueue contribution_queue;
  if (sql.Step()) {
    contribution_queue.id = sql.ColumnString(0);
    contribution_queue.type = static_cast<type::RewardsType>(sql.ColumnInt(1));
  ...
  EXPECT_EQ(contribution_queue.type, type::RewardsType::ONE_TIME_TIP);
  ...
  type::ContributionQueuePublisher queue_publisher;
...
TEST_F(LedgerDatabaseMigrationTest, Migration_26_UnblindedTokens) {
  ...
  type::UnblindedTokenPtr token;
  std::vector<std::string> actual_values;
  while (sql.Step()) {
    if (!token) {
      token = type::UnblindedToken::New();
      ...
      token->redeem_type = static_cast<type::RewardsType>(sql.ColumnInt(8));
    }
  ...
  EXPECT_EQ(token->redeem_type, type::RewardsType::ONE_TIME_TIP);
}
...
TEST_F(LedgerDatabaseMigrationTest, Migration_27_UnblindedTokens) {
  ...
  type::UnblindedToken unblinded_token;
